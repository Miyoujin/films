...
public interface EvaluationDao extends PagingAndSortingRepository<Evaluation,Integer>{
	...
	public List<Evaluation> findByPointsGreaterThanEqual(Integer points);
	 ...
	 //@Query(value = "select new com.dbg.model.evaluation.Evaluation(e.id, e.points, e.user, e.film) from Evaluation as e where (:idUser is null OR e.user.id = :idUser) AND (:idFilm is null OR e.Film.id :idFilm)")
	 //List<Evaluation> findByUserAndFilm(@Param(value = "idUser") Integer idUser, @Param(value = "idFilm") Integer idFilm);
